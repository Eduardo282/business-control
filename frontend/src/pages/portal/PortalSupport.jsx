    ...
    <div className="max-w-4xl mx-auto relative">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Chat en tiempo real con nuestro equipo</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <span className={`inline-block w-2.5 h-2.5 rounded-full ${connected ? "bg-emerald-500 animate-pulse" : "bg-gray-300"}`} />
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/60 border border-gray-100 overflow-hidden">
        ...
        {chatState === "idle" && (
          <div className="p-12 flex flex-col items-center text-center">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-emerald-50 to-teal-50 flex items-center justify-center mb-6 ring-8 ring-emerald-50/50">
              <MessageCircle size={40} className="text-emerald-600" />
            </div>
            <h3 className="text-xl font-bold text-gray-800 mb-2">¿Necesitas ayuda?</h3>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8 w-full max-w-sm">
              {[
                ...
                <div key={i} className="bg-gray-50 rounded-2xl p-4 flex flex-col items-center gap-1">
        ...
        {chatState === "waiting" && (
          <div className="p-12 flex flex-col items-center text-center">
            <div className="relative mb-8">
              ...
              <div className="absolute inset-0 w-20 h-20 rounded-full border-2 border-amber-400 animate-ping opacity-20" style={{ animationDelay: "0.5s" }} />
            </div>
            <h3 className="text-lg font-bold text-gray-800 mb-2">Buscando un agente disponible...</h3>
            <p className="text-sm text-gray-500 mb-4">Un agente se conectará contigo en unos momentos.</p>
          ...
          <>
            ...
            <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center backdrop-blur-sm relative">
                  <Headphones size={20} color="white" />
                  {chatState === "active" && (
                    <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-emerald-600 ${agentOnline ? "bg-green-400" : "bg-gray-400"}`} />
                  )}
                </div>
                <div>
                  ...
                  <span className="text-emerald-100 text-xs flex items-center gap-1.5">
                    {chatState === "active" && (
                      <>
                        <span className={`w-1.5 h-1.5 rounded-full inline-block ${agentOnline ? "bg-green-300 animate-pulse" : "bg-gray-300"}`} />
            ...
            {/* Messages Area */}
            <div
              className="h-[420px] overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-gray-50/50 to-white"
                ...
                if (isSystem) {
                  return (
                    <div key={msg.id} className="flex justify-center">
                      <div className="bg-gray-100 text-gray-500 text-xs px-4 py-2 rounded-full max-w-sm text-center">
                        {msg.body}
                      </div>
                    </div>
                  );
                  ...
                  >
                    <div className={`max-w-[75%] ${isClient ? "order-1" : ""}`}>
                      <div className={`flex items-end gap-2 ${isClient ? "flex-row-reverse" : ""}`}>
                        {/* Avatar */}
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${isClient ? "bg-gradient-to-br from-emerald-500 to-teal-600 text-white" : "bg-gradient-to-br from-blue-500 to-indigo-600 text-white"}`}>
                          {isClient ? (contact?.full_name?.[0] || "C") : <Headphones size={14} />}
                        </div>

                        {/* Bubble */}
                        <div className="relative">
                          <div className={`px-4 py-3 rounded-2xl shadow-sm ${isClient ? "bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-br-md" : "bg-white text-gray-800 border border-gray-100 rounded-bl-md"}`}>
                          ...
                          {isClient && chatState === "active" && hoveredMsgId === msg.id && (
                            <button
                              ...
                              className="absolute -left-8 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-red-100 hover:bg-red-200 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
                      ...
                      {/* Timestamp */}
                      <div className={`text-[10px] text-gray-400 mt-1 ${isClient ? "text-right mr-10" : "ml-10"}`}>
            ...
            {/* ── Rating (when closed) ── */}
            {chatState === "closed" && (
              <div className="px-6 py-6 border-t border-gray-100 bg-gray-50/80 text-center">
                {!rated ? (
                  <>
                    ...
                    <div className="flex justify-center gap-1 mb-4">
                      {[1, 2, 3, 4, 5].map((v) => (
                        <button key={v} onClick={() => handleRate(v)} onMouseEnter={() => setHoverRating(v)} onMouseLeave={() => setHoverRating(0)} className="p-1 transition-transform hover:scale-125">
                          <Star size={28} className={`transition-colors ${v <= (hoverRating || rating) ? "text-amber-400" : "text-gray-200"}`} strokeWidth={v <= (hoverRating || rating) ? 2.5 : 1.5} />
                        </button>
                      ))}
                    </div>
                  ...
                  <>
                    <div className="flex justify-center gap-1 mb-3">
                      {[1, 2, 3, 4, 5].map((v) => (
                        <Star key={v} size={24} className={v <= rating ? "text-amber-400" : "text-gray-200"} strokeWidth={v <= rating ? 2.5 : 1.5} />
                      ))}
                    </div>
                    <p className="text-sm text-gray-600 mb-4">¡Gracias por tu calificación!</p>
            ...
            {/* ── Input Area ── */}
            {chatState === "active" && (
              <form onSubmit={sendMessage} className="border-t border-gray-100 px-4 py-3 flex items-center gap-3 bg-white">
                <input
                  ref={inputRef}
                  type="text"
                  value={inputText}
                  ...
                  className="flex-1 px-4 py-3 bg-gray-50 rounded-xl text-sm border border-gray-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 transition-all placeholder:text-gray-400"
                  autoFocus
                />
                <button type="submit" disabled={!inputText.trim()} className="w-11 h-11 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white flex items-center justify-center shadow-md shadow-emerald-500/20 hover:shadow-lg hover:scale-105 active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100">
                  <Send size={18} />
                </button>
              </form>
            )}
            ...
            {chatState === "waiting" && (
              <div className="border-t border-gray-100 px-4 py-3 bg-gray-50">
                <div className="px-4 py-3 bg-gray-100 rounded-xl text-sm text-gray-400 text-center">
      ...
      <div className="mt-4 text-center">
        <p className="text-xs text-gray-400">Los mensajes se guardan de forma segura para tu referencia futura.</p>
      </div>
    </div>
  );
}
