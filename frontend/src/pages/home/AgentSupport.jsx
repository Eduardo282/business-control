...
export default function AgentSupport() {
  ...
  const shouldHideSystemMessage = (msg) => {
    if (msg?.sender_type !== "SYSTEM") return false;
    const text = String(msg.body || "").trim();
    return (
      text.startsWith("Bienvenido al chat de soporte") ||
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-dark-800 rounded-2xl border border-zinc-100 dark:border-white/10 shadow-md overflow-hidden">
          {!selectedConvId ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-sm text-zinc-400 dark:text-zinc-500 max-w-xs">Toma un chat de la lista de espera o selecciona uno activo.</p>
            </div>
          ) : (
            <>
              {/* Header */}
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3 bg-gradient-to-b from-zinc-50/50 to-white dark:from-dark-800 dark:to-dark-800" style={{ scrollbarWidth: "thin", scrollbarColor: "#d1d5db transparent" }}>
                {messages.map((msg) => {
                  ...
                  if (isSystem) {
                    if (shouldHideSystemMessage(msg)) return null;
                    return (
                      <div key={msg.id} className="flex justify-center">
                        <div className="bg-zinc-100 dark:bg-white/10 text-zinc-500 dark:text-zinc-400 text-xs px-4 py-1.5 rounded-full">
                          {msg.body}
                        </div>
                      </div>
                    );
